...
def main() -> int:
    ...
    raw_root = repo_root / "data" / "raw" / "pts_artifacts"
    derived_root = repo_root / "data" / "derived" / "pts" / "artifact_analysis"
    curated_root = repo_root / "data" / "curated" / "pts" / "artifact_analysis"
    scratch_root = repo_root / "tmp" / "pts_artifact_analysis"
    pts_files = Path(args.pts_files).resolve()
    out_dir = derived_root / "payload_investigation"
    out_dir.mkdir(parents=True, exist_ok=True)
    curated_payload_dir = curated_root / "payload_investigation"
    curated_payload_dir.mkdir(parents=True, exist_ok=True)
    ...
    for root in [
        scratch_root / "rawscan",
        raw_root / "extracted" / "rawscan_2",
        raw_root / "extracted" / "rawscan_3",
        raw_root / "scan_workspace" / "expanded_archives",
    ]:
        if root.exists():
            setup_related_exes.extend([p for p in root.rglob("*.exe") if p.is_file()])
    setup_related_exes = sorted(set(setup_related_exes))

    burn_rows = find_burn_signatures(setup_related_exes, repo_root, out_dir / "burn_signature_hits.tsv")
    ...
    list_payload_files(payload_files, repo_root, out_dir / "payload_msi_cab_list.tsv")

    msi_cab_rows = process_msi_cab(
        payload_files, repo_root, out_dir, args.msiinfo, args.msiextract, args.cabextract
    )
    write_tsv(
    ...
    ets_inventory, ets_with_hits = collect_ets_xml_inventory(ets_search_roots, repo_root, out_dir)
    ...
    md.append(f"- setup exe checked: `{rel(setup_exe, repo_root)}`")
    md.append(f"- files checked for burn tokens: **{len(setup_related_exes)}**")
    md.append(f"- files with burn token hits: **{len(burn_files)}**")
    md.append(f"- detailed hits: `data/derived/pts/artifact_analysis/payload_investigation/burn_signature_hits.tsv`")
    ...
    md.append(f"- payload list (size+path): `data/derived/pts/artifact_analysis/payload_investigation/payload_msi_cab_list.tsv`")
    md.append(f"- MSI/CAB tool status: `data/derived/pts/artifact_analysis/payload_investigation/msi_cab_tool_status.tsv`")
    ...
    md.append(f"- inventory: `data/derived/pts/artifact_analysis/payload_investigation/ets_xml_inventory.tsv`")
    md.append(f"- hits: `data/derived/pts/artifact_analysis/payload_investigation/ets_xml_with_tcids.tsv`")
    md.append(f"- prefix counts from ETS/XML hits: `data/derived/pts/artifact_analysis/payload_investigation/ets_xml_prefix_counts.tsv`")
    md.append("")
    md.append("## Firmware EXE Extra Pass")
    md.append(f"- firmware exe checked: `{rel(firmware_exe, repo_root)}`")
    md.append(f"- summary: `data/derived/pts/artifact_analysis/payload_investigation/firmware_summary.tsv`")
    md.append("")
    (curated_payload_dir / "investigation_summary.md").write_text("\n".join(md), encoding="utf-8")
    ...
    print(f"summary={curated_payload_dir / 'investigation_summary.md'}")
    return 0
