...
def main() -> int:
    ...
    report_root = repo_root / "data" / "derived" / "pts" / "artifact_analysis" / "payload_investigation"
    curated_payload_root = repo_root / "data" / "curated" / "pts" / "artifact_analysis" / "payload_investigation"
    scratch_root = repo_root / "tmp" / "pts_artifact_analysis"
    report_root.mkdir(parents=True, exist_ok=True)
    curated_payload_root.mkdir(parents=True, exist_ok=True)

    msiinfo = "/usr/local/bin/msiinfo"
    msi_path = report_root / "setup_scan" / "rawscan" / "5.msi"
    if not msi_path.exists():
        msi_path = scratch_root / "rawscan" / "5.msi"
    ...
    msi_summary_md = curated_payload_root / "msi_tables_summary.md"
    ...
    cab_md = curated_payload_root / "cab_inventory_top_files.md"
    ...
    cmd.append(f"- קובץ מלא לכל הקבצים: `data/derived/pts/artifact_analysis/payload_investigation/cab_inventory_full.tsv`")
    cmd.append(f"- Top 30 לכל CAB: `data/derived/pts/artifact_analysis/payload_investigation/cab_inventory_top30.tsv`\n")
    ...
    final_md = curated_payload_root / "final_conclusion.md"
    ...
    concl.append("\n## ראיות תומכות")
    concl.append(f"- MSI summary: `data/curated/pts/artifact_analysis/payload_investigation/msi_tables_summary.md`")
    concl.append(f"- CAB inventory: `data/curated/pts/artifact_analysis/payload_investigation/cab_inventory_top_files.md`")
    concl.append(f"- ETS string hits: `data/derived/pts/artifact_analysis/payload_investigation/string_hits_ets_and_paths.tsv`")
    concl.append(f"- Download indicators: `data/derived/pts/artifact_analysis/payload_investigation/burn_download_indicators.tsv`")
    ...
    print(f"derived_outputs={report_root}")
    print(f"curated_outputs={curated_payload_root}")
    return 0
