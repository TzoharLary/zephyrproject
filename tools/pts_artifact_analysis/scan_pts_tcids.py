...
def main() -> int:
    ...
    reports_dir = repo_root / "data" / "derived" / "pts" / "artifact_analysis"
    scratch_root = repo_root / "tmp" / "pts_artifact_analysis"
    reports_dir.mkdir(parents=True, exist_ok=True)
    expanded_dir = raw_root / "scan_workspace" / "expanded_archives"
    if expanded_dir.exists():
        shutil.rmtree(expanded_dir)
    expanded_dir.mkdir(parents=True, exist_ok=True)

    extracted_root = raw_root / "extracted"
    rawscan_root = scratch_root / "rawscan"
    ...
    tcid_to_sources, file_to_tcids = parse_matches(raw_matches_file, repo_root)
    build_reports(reports_dir, repo_root, tcid_to_sources)
    ...
    for path, meta in sorted(files_map.items(), key=lambda x: str(x[0])):
        inventory_rows.append(
            {
                "path": workspace_rel(path, repo_root),
                "root": meta.root,
                "depth": str(meta.depth),
                "previous_scope": "yes" if meta.previous_scope else "no",
                "extracted_from": workspace_rel(meta.extracted_from, repo_root)
                if meta.extracted_from
                else "",
                "archive_candidate": "yes" if is_archive_candidate(path) else "no",
                "tcid_hits": str(len(file_to_tcids.get(workspace_rel(path, repo_root), set()))),
            }
        )
