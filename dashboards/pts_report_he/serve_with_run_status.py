...
GROUP_B_TEST_NOTES_FILE = ROOT_DIR / "autopts" / "data" / "group-b-test-notes-state.json"
GROUP_B_TEST_NOTES_API_PATH = "/api/group-b-test-notes"
PRIMARY_PAGE_PATH = "/autopts/index.html"
MAIN_DASHBOARD_PATH = "/index.html"
SCHEMA_VERSION = 1
GROUP_B_TASKS_SCHEMA_VERSION = 1
GROUP_B_TEST_NOTES_SCHEMA_VERSION = 1
...
def normalize_group_b_test_notes_payload(payload: Any) -> Dict[str, Any]:
    if not isinstance(payload, dict):
        raise ValueError("Payload must be a JSON object")
    profiles = payload.get("profiles")
    if profiles is None:
    ...
    if not isinstance(profiles, dict):
        raise ValueError("'profiles' must be an object")
    version = payload.get("version", GROUP_B_TEST_NOTES_SCHEMA_VERSION)
    try:
        version = int(version)
    except Exception as exc:  # noqa: BLE001
        raise ValueError("'version' must be an integer") from exc
    ...
    for pid in GROUP_B_TASKS_PROFILE_IDS:
        ...
        if not isinstance(notes, dict):
            raise ValueError(f"'profiles.{pid}.notes' must be an object")
        norm_notes: Dict[str, Any] = {}
        for row_id, note_entry in notes.items():
            if not isinstance(row_id, str) or not row_id.strip():
                raise ValueError(f"'profiles.{pid}.notes' contains invalid row id")
            if note_entry is None:
                note_entry = {}
            if not isinstance(note_entry, dict):
                raise ValueError(f"'profiles.{pid}.notes.{row_id}' must be an object")
            note_text = str(note_entry.get("note", ""))
            updated_by = str(note_entry.get("updated_by", ""))
            updated_at = note_entry.get("updated_at")
            if not note_text.strip() and not updated_by.strip() and not updated_at:
                continue
            norm_notes[row_id] = {
                "note": note_text,
        ...
        norm_profiles[pid] = {"notes": norm_notes}
...
def ensure_group_b_test_notes_file() -> None:
    if GROUP_B_TEST_NOTES_FILE.exists():
        return
    GROUP_B_TEST_NOTES_FILE.parent.mkdir(parents=True, exist_ok=True)
    seed = {
        "version": GROUP_B_TEST_NOTES_SCHEMA_VERSION,
        "updated_at": None,
        "profiles": {pid: {"notes": {}} for pid in GROUP_B_TASKS_PROFILE_IDS},
    }
    GROUP_B_TEST_NOTES_FILE.write_text(json.dumps(seed, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
...
def read_group_b_test_notes_payload() -> Dict[str, Any]:
    ensure_group_b_test_notes_file()
    raw = json.loads(GROUP_B_TEST_NOTES_FILE.read_text(encoding="utf-8"))
    return normalize_group_b_test_notes_payload(raw)
...
def write_group_b_test_notes_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    normalized = normalize_group_b_test_notes_payload(payload)
    normalized["updated_at"] = datetime.now(timezone.utc).isoformat()
    GROUP_B_TEST_NOTES_FILE.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.NamedTemporaryFile(
        mode="w",
        encoding="utf-8",
        dir=str(GROUP_B_TEST_NOTES_FILE.parent),
        prefix="group-b-test-notes-",
        suffix=".tmp",
        delete=False,
    ) as tmp:
        tmp.write(json.dumps(normalized, ensure_ascii=False, indent=2) + "\n")
        tmp_path = Path(tmp.name)
    tmp_path.replace(GROUP_B_TEST_NOTES_FILE)
    return normalized
...
class DashboardHandler(SimpleHTTPRequestHandler):
    ...
    def do_OPTIONS(self) -> None:  # noqa: N802
        if self.path not in {API_PATH, GROUP_B_TASKS_API_PATH, GROUP_B_TEST_NOTES_API_PATH}:
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if self.path == GROUP_B_TASKS_API_PATH:
            ...
            self._write_json(HTTPStatus.OK, payload)
            return
        if self.path == GROUP_B_TEST_NOTES_API_PATH:
            try:
                payload = read_group_b_test_notes_payload()
            except Exception as exc:  # noqa: BLE001
                self._write_json(
                    HTTPStatus.INTERNAL_SERVER_ERROR,
                    {"ok": False, "error": f"Failed to read group-b test notes file: {exc}"},
                )
                return
            self._write_json(HTTPStatus.OK, payload)
            return
    ...
    def do_PUT(self) -> None:  # noqa: N802
        ...
        try:
            ...
            elif self.path == GROUP_B_TASKS_API_PATH:
                saved = write_group_b_tasks_payload(payload)
            else:
                saved = write_group_b_test_notes_payload(payload)
        ...
        if self.path == GROUP_B_TASKS_API_PATH:
            ...
            return
        notes_profiles = saved.get("profiles") if isinstance(saved, dict) else {}
        notes_count = 0
        if isinstance(notes_profiles, dict):
            for pid in GROUP_B_TASKS_PROFILE_IDS:
                notes = ((notes_profiles.get(pid) or {}).get("notes") if isinstance(notes_profiles.get(pid), dict) else {})
        ...
        self._write_json(
            ...
            {
                "ok": True,
                "file": str(GROUP_B_TEST_NOTES_FILE.relative_to(ROOT_DIR)),
                "updated_at": saved.get("updated_at"),
                "profiles_count": len(GROUP_B_TASKS_PROFILE_IDS),
...
def main() -> None:
    ...
    print(f"Group B test notes API: {root_url}api/group-b-test-notes")
    print(f"File-backed storage: {RUN_STATUS_FILE}")
    print(f"Group B tasks file: {GROUP_B_TASKS_FILE}")
    print(f"Group B test notes file: {GROUP_B_TEST_NOTES_FILE}")
    if not args.no_open:
        try:
